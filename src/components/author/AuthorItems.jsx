          ...
          //it renders the skeletonLoading placeholder.
            ...
            : authorData.nftCollection.map((item, index) => (
                ...
                <div
                  ...
                  key={index}
                >
                  <div className="nft__item"> {/* Container for each NFT item */}
                    <div className="author_list_pp"> {/* Profile Picture */}
                      <Link to="">{/* Clickable link (empty)wrapping the author's profile picture */}
                        <img
                          className="lazy"
                    ...
                    <div className="nft__item_wrap">
                      <Link to={`/item-details/${item.nftId}`}> {/* Link to the details page for the specific NFT item, using its nftId */}
                        <img
                          src={item.nftImage}
                    ...
                    <div className="nft__item_info"> {/* Container NFT's metadata  title, ETH, likes */}
                      <Link to={`/item-details/${item.nftId}`}> {/* link to the details page for the specific NFT item, using its nftId */}
                        <h4>{item.title}</h4> {/*Title */}
                      </Link>
                      <div className="nft__item_price">{item.price} ETH</div> {/* Displays the price of NFT  in ETH */}
